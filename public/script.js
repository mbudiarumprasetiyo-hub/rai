...
async function sendMessage() {
    ...
    console.log("RAI IMAGE PREVIEW REMOVED");
    ...
    try {
        ...
        console.log("RAI BEFORE JSON STRINGIFY");

        let raiPayload;
        try {
            raiPayload = JSON.stringify({
            ...
            console.error("RAI JSON ERROR:", e);
            throw e;
        }

        console.log("RAI BEFORE FETCH");
...
// ========================================
// RAI TRACK IMAGE
// Baca EXIF + GPS dari foto
// ========================================

function openTrackImage() {
    const input = document.getElementById("trackImageInput");

    if (!input) {
        console.error("trackImageInput tidak ditemukan");
        return;
    }
...
async function trackImageSelected(event) {
    ...
    const button = document.getElementById("trackImageBtn");
    ...
    try {
        ...
        // Kirim FILE ASLI langsung.
        // Tidak memakai FileReader / Base64 agar EXIF GPS tetap utuh.
        const formData = new FormData();
        formData.append("image", file, file.name);

        console.log("RAI TRACK UPLOAD:", {
            name: file.name,
            type: file.type,
            size: file.size
        });

        const response = await fetch("/api/track-image", {
            method: "POST",
            body: formData
        });
        ...
        if (!response.ok || !result.success) {
            throw new Error(
                result.error ||
                "Gagal membaca metadata gambar."
            );
        }

        console.log("RAI TRACK RESULT:", result);

        showTrackImageResult(result, file.name);
...
function showTrackImageResult(result, fileName) {
    ...
    html += `
        <div class="track-image-result">
            <div class="track-title">
                📍 TRACK IMAGE
            </div>

            <div class="track-file">
                ${escapeHtmlSafe(fileName || "Gambar")}
            </div>
    `;
    ...
    if (gps) {
        ...
        html += `
            <div class="track-section">
                <div class="track-label">GPS LOCATION</div>
        ...
        html += `
            <div class="track-section track-no-gps">
                <div class="track-label">GPS LOCATION</div>

                <div class="track-value">
                    GPS tidak tersedia di metadata foto.
                </div>

                <div class="track-note">
                    RAI tidak menebak lokasi dari gambar.
                </div>
            </div>
        `;
    }
    ...
    if (device.manufacturer || device.model) {
        html += `
            <div class="track-section">
                <div class="track-label">DEVICE</div>
    ...
    if (camera.iso !== null && camera.iso !== undefined) {
        cameraRows.push(`ISO: ${escapeHtmlSafe(String(camera.iso))}`);
    }

    if (camera.aperture !== null && camera.aperture !== undefined) {
        cameraRows.push(
            `Aperture: f/${escapeHtmlSafe(String(camera.aperture))}`
        );
    }

    if (camera.shutter !== null && camera.shutter !== undefined) {
        cameraRows.push(
            `Shutter: ${escapeHtmlSafe(String(camera.shutter))}`
        );
    }

    if (camera.focalLength !== null && camera.focalLength !== undefined) {
        cameraRows.push(
            `Focal length: ${escapeHtmlSafe(String(camera.focalLength))} mm`
        );
    }
    ...
    if (cameraRows.length) {
        html += `
            <div class="track-section">
                <div class="track-label">CAMERA DATA</div>
    ...
    if (result.dateTaken) {
        html += `
            <div class="track-section">
                <div class="track-label">CAPTURE DATE</div>

                <div class="track-value">
                    ${escapeHtmlSafe(String(result.dateTaken))}
                </div>
            </div>
        `;
    }

    if (result.gpsAltitude !== null &&
        result.gpsAltitude !== undefined) {

        html += `
            <div class="track-section">
                <div class="track-label">ALTITUDE</div>

                <div class="track-value">
                    ${escapeHtmlSafe(String(result.gpsAltitude))} m
                </div>
            </div>
        `;
    }
...
function addTrackResultToChat(html) {
    const chat =
    ...
    if (!chat) {
        console.warn("Container chat tidak ditemukan.");

        alert(
            "TRACK IMAGE berhasil membaca metadata, " +
            "tetapi area chat RAI tidak ditemukan."
        );
    ...
    wrapper.className = "rai-message assistant track-image-message";
...
function openTrackMap(latitude, longitude) {
    if (
